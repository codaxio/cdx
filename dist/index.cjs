...
var executeSync = async (command, options = { cwd: process.cwd() }) => {
  const state = {
    stdout: "",
    stderr: "",
    err: null,
...
var execute = async (command, options = { cwd: process.cwd() }) => {
...
var readJson = (path, def = {}) => {
  try {
    return JSON.parse(import_fs.default.readFileSync(path).toString());
  } catch (e) {
    return def;
  }
};
...
var c = {
  ...
  bgBlue: source_default.bgBlue,
  bgRed: source_default.bgRed,
  bgGreen: source_default.bgGreen,
  bgYellow: source_default.bgYellow,
  bgCyan: source_default.bgCyan,
  bgMagenta: source_default.bgMagenta,
  bgWhite: source_default.bgWhite,
  bgGray: source_default.bgGray,
  bgBlack: source_default.bgBlack,
  bgRgb: source_default.bgRgb,
  rgb: source_default.rgb,
  hex: source_default.hex,
  bgHex: source_default.bgHex
};

// src/command.ts
var BaseCommand = class {
  constructor(program2, config) {
    this.program = program2;
    this.config = config;
    __publicField(this, "name", "BaseCommand");
    __publicField(this, "description", "Command");
...
// src/cli.ts
async function cli() {
  const program2 = new Command().name("cdx").version(package_default.version).description("CDX CLI").option("-w, --cwd <path>", "Teleport to this directory").option("-c, --config <config>", "Config file").option("-l, --load <path...>", "Load commands from theses dirs or files").action(async (options, command) => {
    dd("CDX CLI", options);
    ...
    const program3 = new Command().command("cdx").description("Cozy Developer eXperience.").action(async (data, command2, c2, d) => program3.help());
    ...
    let registration = await Promise.all(commands.map(async (cmd) => {
      ...
      let instance = new Command3(program3, config);
